...
def listar_categorias():
    ...
    return render_template('categorias/categorias_list.html', receitas=receitas, despesas=despesas)
...
def nova_categoria():
    if request.method == 'POST':
        ...
        icone = request.form.get('icon', '')  # Corrigido para pegar o ícone
        nova = Category(name=nome, type=tipo, icon=icone)
        db.session.add(nova)
        db.session.commit()
        return redirect(url_for('categories.listar_categorias'))
    return render_template('categorias/categorias_new.html')
...
def editar_categoria(id):
    ...
    if request.method == 'POST':
        ...
        categoria.icon = request.form.get('icon', categoria.icon)  # Adicionado para permitir edição do ícone
        db.session.commit()
        return redirect(url_for('categories.listar_categorias'))
    return render_template('categorias/categorias_edit.html', categoria=categoria)
